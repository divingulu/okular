...
class SettingsCoreHelper
{
  public:
    ...
    ~SettingsCoreHelper() { delete q; q = nullptr; }
...
void SettingsCore::setPaperColor( const QColor & v )
{
    if (v != self()->d->paperColor && !self()->SettingsCore::isPaperColorImmutable()) {
      self()->d->paperColor = v;
      self()->d->settingsChanged.insert(signalColorModesChanged);
    }
}
...
{
  delete d;
  if (s_globalSettingsCore.exists() && !s_globalSettingsCore.isDestroyed()) {
...
bool SettingsCore::usrSave()
{
  ...
  if (d->settingsChanged.contains(signalColorModesChanged))
    Q_EMIT colorModesChanged();
  d->settingsChanged.clear();
  return true;
}

void SettingsCore::itemChanged(quint64 signalFlag) {
