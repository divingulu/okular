...
public:
	explicit PickPointEngine(const QDomElement &engineElement)
		...
		if (m_annotElement.attribute(QStringLiteral("type")) == QLatin1String("Stamp") &&
				!iconName.simplified().isEmpty()) {
	...
	QRect event(EventType type, Button button, Modifiers modifiers, double nX, double nY, double xScale, double yScale,
							const Okular::Page *page) override {
		xscale = xScale;
		yscale = yScale;
		...
		if (m_block) {
			const Okular::NormalizedRect tmprect(qMin(startpoint.x, point.x), qMin(startpoint.y, point.y),
																					 qMax(startpoint.x, point.x), qMax(startpoint.y, point.y));
	...
	//TODO 为什么调用setContents
	void addInPlaceTextAnnotation(Okular::Annotation *&ann, const QString &summary, const QString &content,
																Okular::TextAnnotation::InplaceIntent inplaceIntent) {
		...
		const QRectF rcf =
						mf.boundingRect(Okular::NormalizedRect(rect.left, rect.top, 1.0, 1.0).geometry((int) pagewidth,
																																													 (int) pageheight).adjusted(
																		padding, padding, -padding, -padding), Qt::AlignTop | Qt::AlignLeft | Qt::TextWordWrap,
														ta->contents());
	...
	QList<Okular::Annotation *> end() override {
		...
		if (typeString == QLatin1String("FreeText")) {
			...
			bool resok;
			const QString content = QInputDialog::getMultiLineText(nullptr, i18n("New Text Note"),
																														 i18n("Text of the new note:"), QString(), &resok);
			if (resok) {
		...
		ann->style().setColor(
						m_annotElement.hasAttribute(QStringLiteral("color")) ? m_annotElement.attribute(QStringLiteral("color"))
																																 : m_engineColor);
...
public:
	PickPointEngineSignature(Okular::Document *document, PageView *pageView)
					: PickPointEngine({}), m_document(document), m_page(nullptr), m_pageView(pageView), m_startOver(false),
						m_aborted(false) {
		m_block = true;
	}
	...
	QList<Okular::Annotation *> end() override {
		...
		if (rect.width() * m_page->width() < 100 || rect.height() * m_page->height() < 100) {
			const KMessageBox::ButtonCode answer = KMessageBox::questionYesNo(
							m_pageView,
							xi18nc("@info",
										 "A signature of this size may be too small to read. If you would like to create a potentially more readable signature, press <interface>Start over</interface> and draw a bigger rectangle."),
							QString(),
		...
		const auto signInfo = SignaturePartUtils::getCertificateAndPasswordForSigning(m_pageView, m_document,
																																									SignaturePartUtils::SigningInformationOption::BackgroundImage);
		if (!signInfo) {
			m_aborted = true;
			passToUse.clear();
			...
			certCommonName = signInfo->certificate->subjectInfo(Okular::CertificateInfo::CommonName,
																													Okular::CertificateInfo::EmptyString::TranslatedNotAvailable);
...
public:
	...
	static Okular::NormalizedPoint
	constrainAngle(const Okular::NormalizedPoint &p1, double x, double y, double xScale, double yScale,
	...
	QRect event(EventType type, Button button, Modifiers modifiers, double nX, double nY, double xScale, double yScale,
		...
		if (modifiers.constrainRatioAndAngle && !points.isEmpty()) {
			const Okular::NormalizedPoint constrainedPoint = constrainAngle(points.constLast(), nX, nY, xScale, yScale,
																																			M_PI / 12.);
		...
		if (type == Press) {
			...
			if (numofpoints == -1 && points.count() > 1 &&
					(fabs(points[0].x - newPoint.x) + fabs(points[0].y - newPoint.y) < 1e-2)) {
				last = true;
			} else {
	...
	QList<Okular::Annotation *> end() override {
		...
		if (typeString == QLatin1String("Line") || typeString == QLatin1String("Polyline") ||
				typeString == QLatin1String("Polygon")) {
			if (points.count() < 2) {
				return QList<Okular::Annotation *>();
			}
			...
			if (m_annotElement.hasAttribute(QStringLiteral("startStyle"))) {
				la->setLineStartStyle(
								(Okular::LineAnnotation::TermStyle) m_annotElement.attribute(QStringLiteral("startStyle")).toInt());
			}
			if (m_annotElement.hasAttribute(QStringLiteral("endStyle"))) {
				la->setLineEndStyle(
								(Okular::LineAnnotation::TermStyle) m_annotElement.attribute(QStringLiteral("endStyle")).toInt());
			}
...
public:
	...
	QRect
	event(EventType type, Button button, Modifiers /*modifiers*/, double nX, double nY, double xScale, double yScale,
				const Okular::Page * /*page*/) override {
		// only proceed if pressing left button
		if (button != Left) {
			return QRect();
		...
		if (type == Press) {
			...
			if (item()) {
				const QPoint start((int) (lastPoint.x * item()->uncroppedWidth()),
													 (int) (lastPoint.y * item()->uncroppedHeight()));
...
PageViewAnnotator::PageViewAnnotator(PageView *parent, Okular::Document *storage)
				: QObject(parent), m_document(storage), m_pageView(parent), m_actionHandler(nullptr), m_engine(nullptr),
					m_builtinToolsDefinition(nullptr), m_quickToolsDefinition(nullptr), m_continuousMode(true),
					m_constrainRatioAndAngle(false), m_signatureMode(false), m_lastToolsDefinition(nullptr), m_lastToolId(-1),
					m_lockedItem(nullptr) {
	reparseConfig();
	reparseBuiltinToolsConfig();
	reparseQuickToolsConfig();
	connect(Okular::Settings::self(), &Okular::Settings::builtinAnnotationToolsChanged, this,
					&PageViewAnnotator::reparseBuiltinToolsConfig);
	connect(Okular::Settings::self(), &Okular::Settings::quickAnnotationToolsChanged, this,
					&PageViewAnnotator::reparseQuickToolsConfig, Qt::QueuedConnection);
}
...
//		detachAnnotation();
	...
	const QRect paintRect = m_engine->event(eventType, button, modifiers, nX, nY, itemRect.width(), itemRect.height(),
																					m_lockedItem->page());
			...
			if (signEngine->isAccepted()) {
				...
				if (!newFilePath.isEmpty()) {
					...
					if (success) {
						...
						KMessageBox::error(m_pageView, i18nc("%1 is a file path",
...
void PageViewAnnotator::selectTool(AnnotationTools *toolsDefinition, int toolId, ShowTip showTip) {
	...
	if (toolId > 0 && Okular::Settings::identityAuthor().isEmpty()) {
		...
		if (userName.isEmpty()) {
			bool ok = false;
			userName = QInputDialog::getText(nullptr, i18n("Author name"), i18n("Author name for the annotation:"),
...
QPixmap PageViewAnnotator::makeToolPixmap(const QDomElement &toolElement) {
	...
	pixmap.load(QStandardPaths::locate(QStandardPaths::GenericDataLocation,
																		 QStringLiteral("okular/pics/tool-base-okular") + imageVariant +
																		 QStringLiteral(".png")));
	...
	if (annotType == QLatin1String("ellipse")) {
		...
		QImage overlay(QStandardPaths::locate(QStandardPaths::GenericDataLocation,
																					QStringLiteral("okular/pics/tool-highlighter-okular-colorizable") +
																					imageVariant + QStringLiteral(".png")));
		...
		QImage overlay(QStandardPaths::locate(QStandardPaths::GenericDataLocation,
																					QStringLiteral("okular/pics/tool-ink-okular-colorizable") + imageVariant +
																					QStringLiteral(".png")));
		QImage colorizedOverlay = overlay;
		GuiUtils::colorizeImage(colorizedOverlay, engineColor);
		...
		QImage overlay(QStandardPaths::locate(QStandardPaths::GenericDataLocation,
																					QStringLiteral("okular/pics/tool-note-inline-okular-colorizable") +
																					imageVariant + QStringLiteral(".png")));
		GuiUtils::colorizeImage(overlay, engineColor);
		p.drawImage(QPoint(0, 0), overlay);
	} else if (annotType == QLatin1String("note-linked")) {
		QImage overlay(QStandardPaths::locate(QStandardPaths::GenericDataLocation,
																					QStringLiteral("okular/pics/tool-note-okular-colorizable") + imageVariant +
																					QStringLiteral(".png")));
		GuiUtils::colorizeImage(overlay, engineColor);
		p.drawImage(QPoint(0, 0), overlay);
	} else if (annotType == QLatin1String("polygon")) {
		...
		QImage overlay(QStandardPaths::locate(QStandardPaths::GenericDataLocation,
																					QStringLiteral("okular/pics/tool-typewriter-okular-colorizable") +
																					imageVariant + QStringLiteral(".png")));
...
void PageViewAnnotator::addToQuickAnnotations() {
	...
	bool ok = false;
	QString itemText = QInputDialog::getText(nullptr, i18n("Add favorite annotation"), i18n("Custom annotation name:"),
																					 QLineEdit::Normal, defaultToolName(sourceToolElement), &ok);
	if (!ok) {
		return;
	}
